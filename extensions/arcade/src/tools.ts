 ...
 *
 * Converts Arcade tool definitions to Moltbot tools and handles
...
export type ArcadeToolContext = {
  client: ArcadeClient;
  config: ArcadeConfig;
  logger: MoltbotPluginApi["logger"];
};
...
/**
 * Convert Arcade tool name to Moltbot tool name
 * Arcade: "Gmail.SendEmail" -> Moltbot: "arcade_gmail_send_email"
 */
export function toMoltbotToolName(arcadeName: string, prefix: string): string {
...
/**
 * Convert Moltbot tool name back to Arcade tool name
 */
export function toArcadeToolName(moltbotName: string, prefix: string): string {
  // Remove prefix
  const withoutPrefix = moltbotName.replace(new RegExp(`^${prefix}_`), "");
...
/**
 * Create a Moltbot tool from an Arcade tool definition
 */
export function createMoltbotTool(
  arcadeTool: ArcadeToolDefinition,
  ctx: ArcadeToolContext,
) {
  const { client, config, logger } = ctx;
  const moltbotName = toMoltbotToolName(arcadeTool.name, config.toolPrefix);
...
/**
 * Register all available Arcade tools with Moltbot
 */
export async function registerArcadeTools(
  ...
  try {
    ...
    for (const arcadeTool of tools) {
      ...
      const toolkitConfig = config.toolkits?.[arcadeTool.toolkit.toLowerCase()];
      ...
      const moltbotTool = createMoltbotTool(arcadeTool, ctx);

      api.registerTool(moltbotTool, { optional: true });

      registered.push({
        name: moltbotTool.name,
        arcadeName: arcadeTool.name,
        toolkit: arcadeTool.toolkit,
        requiresAuth: arcadeTool.requires_auth ?? false,
      });
    }

    api.logger.info(`[arcade] Registered ${registered.length} tools`);
    return registered;
  } catch (err) {
    api.logger.error(
...
export function registerStaticTools(
  ...
  // Define essential tools that don't require API discovery
  const essentialTools: ArcadeToolDefinition[] = [
    {
      name: "arcade.list_tools",
      ...
      parameters: {
        ...
        properties: {
          toolkit: {
            name: "toolkit",
            type: "string",
            description: "Filter by toolkit name (e.g., gmail, slack, github)",
          },
        },
      },
    },
    {
      name: "arcade.authorize",
      ...
      parameters: {
        ...
        properties: {
          tool_name: {
            name: "tool_name",
            type: "string",
            description: "The Arcade tool name (e.g., Gmail.SendEmail)",
            required: true,
          },
        },
        required: ["tool_name"],
    ...
    {
      name: "arcade.execute",
      description: "Execute any Arcade tool by name",
      toolkit: "arcade",
      parameters: {
        type: "object",
        properties: {
