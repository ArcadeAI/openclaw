...
import { toMoltbotToolName, toArcadeToolName } from "./tools.js";

describe("toMoltbotToolName", () => {
  it("converts simple tool names", () => {
    expect(toMoltbotToolName("Gmail.SendEmail", "arcade")).toBe(
      "arcade_gmail_send_email",
    );
  });

  it("converts CamelCase to snake_case", () => {
    expect(toMoltbotToolName("GoogleCalendar.CreateEvent", "arcade")).toBe(
  ...
  it("handles multi-word toolkit names", () => {
    ...
    expect(toMoltbotToolName("MSTeams.SendMessage", "arcade")).toBe(
  ...
  it("handles tools with multiple capital letters", () => {
    ...
    expect(toMoltbotToolName("GitHub.CreatePR", "arcade")).toBe(
      "arcade_git_hub_create_p_r",
    );
  });

  it("uses custom prefix", () => {
    expect(toMoltbotToolName("Gmail.SendEmail", "arc")).toBe(
      "arc_gmail_send_email",
    );
  });

  it("handles single-word tool names", () => {
    expect(toMoltbotToolName("Search.Query", "arcade")).toBe(
      "arcade_search_query",
    );
  });
});
...
describe("tool name round-trip", () => {
  ...
  for (const arcadeName of testCases) {
    it(`converts ${arcadeName} round-trip`, () => {
      const moltbotName = toMoltbotToolName(arcadeName, "arcade");
      const backToArcade = toArcadeToolName(moltbotName, "arcade");
